  ...
  return (
    <div style={{ display: 'flex', gap: 8, marginBottom: 16, overflowX: 'auto', scrollbarWidth: 'none' }}>
      {MARKETS.map(({ sym, label, fmt }, i) => {
        ...
        const color = pct === null ? 'var(--text-secondary)' : pct >= 0 ? 'var(--up)' : 'var(--down)';
        const isUp = (pct ?? 0) >= 0;
        ...
        return (
          ...
          >
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)', whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>
              {m ? fmt(m.price) : <span style={{ color: 'var(--text-secondary)' }}>...</span>}
            </div>
